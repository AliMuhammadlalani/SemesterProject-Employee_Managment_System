...
class UpdateEmployee extends AddEmployee implements ActionListener{
    ...
    void showData(String s){
        try{
            Conn con = new Conn();
            String str = "select * from employee where `Employee Id` = '"+s+"'";
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
                Conn con = new Conn();
                String str = "update employee set Name='"+t1.getText()+"'," +
                        "`Father's Name`='"+t2.getText()+"',Age='"+age+"'," +
                        "`Date of Birth`='"+dat+"',`Address`='"+t3.getText()+"'," +
                        "Phone='"+t4.getText()+"',`Email Id`='"+t5.getText()+"'," +
                        "Education='"+t6.getText()+"',`Job Post`='"+t7.getText()+"'," +
                        "`CNIC No`='"+t8.getText()+"',`Employee Id`='"+t9.getText()+"' " +
                        "where `Employee Id`='"+id_emp+"'";
